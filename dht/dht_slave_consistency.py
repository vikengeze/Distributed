...
def startup(): 
  global node,master_addr,k,consistency
  k = app.config['kreplicas']
  consistency = app.config['consistency']
  master_addr =  app.config['master']
  print(k,consistency,master_addr, end='\n')
  ...
  t0=threading.Thread(target=join_circle, args=(addr,))
  t0.start()
  print("This server's id is {0}".format(node.id))

#Slave nodes send join request to master every time they come alive
def join_circle(addr):
  url = "http://{0}/dht/join?data={1}".format(master_addr, addr)
  r=requests.post(url)
...
def depart():
  ...
  while(True):
    for key in keys:
      val = str(node.my_dict.get(key))
      print(val)
      url = "http://{0}/update_depart?i={1}&key={2}&val={3}".format(succ,i,key,val)
      r = requests.post(url)
    ...
    if r.status_code == 200:
      ...
      abort(404)
    if(succ==node.host or i==k):
      break
    i+=1
  departed = node.depart(k)
  if not departed:
    return abort(404)
  leave()
  return "ok", 200
...
def leave():
  ...
  func()
  print("I departed. My id was {0}. Bye!".format(node.id))
  return "ok", 200

#Helper endpoint for node's depart - updates copies of other nodes
